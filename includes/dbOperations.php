...
class DbOperations
{
	...
	{
		// First, check if the colour already exists
		$stmt = $this->con->prepare("SELECT `colour` FROM `colours` WHERE `colour` = ?");
		...
		if ($result->num_rows > 0) {
			...
			return 3; // You can choose another return value if you prefer
		}

		// If the colour doesn't exist, insert it
	...
	{
		// Check if vehicle_id exists for this user in the wishlist
		$stmt = $this->con->prepare("SELECT `quantity` FROM `wishlist` WHERE `user_id` = ? AND `vehicle_id` = ?");
		$stmt->bind_param("ii", $user_id, $vehicle_id);
		$stmt->execute();
		$result = $stmt->get_result();

		if ($result->num_rows > 0) {
			// Vehicle already in wishlist for this user
			$row = $result->fetch_assoc();

			// Update the quantity
			$newQuantity = $row['quantity'] + $quantity;

			$stmt = $this->con->prepare("UPDATE `wishlist` SET `quantity` = ? WHERE `user_id` = ? AND `vehicle_id` = ?");
			$stmt->bind_param("iii", $newQuantity, $user_id, $vehicle_id);
			...
			// Vehicle not in wishlist for this user, so insert new record
	...
	{
		// Check if vehicle_id exists for this user
		$stmt = $this->con->prepare("SELECT `quantity`, `total_price` FROM `cart` WHERE `user_id` = ? AND `vehicle_id` = ?");
		$stmt->bind_param("ii", $user_id, $vehicle_id);
		$stmt->execute();
		$result = $stmt->get_result();

		if ($result->num_rows > 0) {
			// Vehicle already in cart for this user
			$row = $result->fetch_assoc();

			// Update the quantity and total price
			$newQuantity = $row['quantity'] + $quantity;
			$newTotal = $row['total_price'] + $total;

			$stmt = $this->con->prepare("UPDATE `cart` SET `quantity` = ?, `total_price` = ? WHERE `user_id` = ? AND `vehicle_id` = ?");
			$stmt->bind_param("iiii", $newQuantity, $newTotal, $user_id, $vehicle_id);
			...
			// Vehicle not in cart for this user, so insert new record
	...
	{
		$stmt = $this->con->prepare("SELECT * FROM `vehicles` INNER JOIN `manufacturers` ON manufacturers.make_id = vehicles.make INNER JOIN `colours` ON colours.id = vehicles.colour INNER JOIN `transmissions` ON transmissions.id = vehicles.transmission ORDER BY RAND();");
		$stmt->execute();
		return $stmt->get_result();
	}

	public function getInStockVehicles()
	{
		$stmt = $this->con->prepare("SELECT * FROM `vehicles` INNER JOIN `manufacturers` ON manufacturers.make_id = vehicles.make INNER JOIN `colours` ON colours.id = vehicles.colour INNER JOIN `transmissions` ON transmissions.id = vehicles.transmission WHERE `vehicles`.`in_stock`= 1 ORDER BY RAND();");
		$stmt->execute();
		return $stmt->get_result();
	}
	...
	{
		$sql = "SELECT vehicles.*, manufacturers.*, colours.*, transmissions.*, order_items.*, orders.* FROM `order_items` INNER JOIN `orders` ON orders.order_id = order_items.order_id INNER JOIN `vehicles` ON vehicles.vehicle_id = order_items.vehicle_id 
	...
	{
		$sql = "SELECT vehicles.*, manufacturers.*, colours.*, transmissions.*, order_items.*, orders.* 
            FROM `order_items` 
            ...
            INNER JOIN `colours` ON colours.id = vehicles.colour 
            INNER JOIN `transmissions` ON transmissions.id = vehicles.transmission 
            WHERE orders.user_id = ? AND orders.order_status = 0";
	...
	public function getAllPendingOrders()
	{
		...
		INNER JOIN `vehicles` ON vehicles.vehicle_id = order_items.vehicle_id 
        INNER JOIN `manufacturers` ON manufacturers.make_id = vehicles.make WHERE `order_status` = 0 ORDER BY `timestamp`");
		$stmt->execute();
		return $stmt->get_result();
	}
	...
	{
		$keyword = "%{$keyword}%"; // Prepare the keyword for a LIKE SQL search

		$stmt = $this->con->prepare("SELECT * FROM `vehicles` 
                                 INNER JOIN `manufacturers` ON manufacturers.make_id = vehicles.make 
                                 INNER JOIN `colours` ON colours.id = vehicles.colour 
                                 INNER JOIN `transmissions` ON transmissions.id = vehicles.transmission 
                                 WHERE `vehicles`.`in_stock` = 1 AND ( manufacturers.name LIKE ? OR vehicles.model LIKE ? OR vehicles.price LIKE ? 
								 OR transmissions.transmission_type LIKE ? OR colours.colour LIKE ?)
                                 ORDER BY RAND();");

		$stmt->bind_param('sssss', $keyword, $keyword, $keyword, $keyword, $keyword); // 'ss' indicates that we're binding two string parameters

		$stmt->execute();
		return $stmt->get_result();
	}

	public function findVehicles($sql)
	{
		$stmt = $this->con->prepare($sql);
